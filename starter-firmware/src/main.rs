...
fn main() -> anyhow::Result<()> {
    ...
    let periphierals = Peripherals::take()?;
...
pub struct ExampleServer {
    radio: Arc<Mutex<PinDriver<'static, AnyOutputPin, Output>>>,
    engine: Arc<Mutex<PinDriver<'static, AnyOutputPin, Output>>>,
    ignition: Arc<Mutex<PinDriver<'static, AnyOutputPin, Output>>>,
...
impl ExampleServer {
    pub fn new(
        ...
        Self {
            ...
            engine,
            ignition,
            button_state: Arc::new(Mutex::new(0)),
            gap,
            gatts,
...
impl ExampleServer {
    ...
    fn on_recv(&self, addr: BdAddr, data: &[u8], offset: u16, mtu: Option<u16>) {
        // Put your custom code herea
        let mut state = data[0];
        match state {
            0 => {
                self.radio.lock().unwrap().set_low().unwrap();
                self.engine.lock().unwrap().set_low().unwrap();
            ...
            _ => {
                error!("invalid start state: {state}");
                return;
            }
        }
